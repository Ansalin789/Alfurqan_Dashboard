...
const sortedClasses = allClasses.sort(
 ...
 const filteredData = uniqueStudentSchedules.filter((row) => {
  ...
  return (
    <BaseLayout>
      <TeacherHeader currentSection="My Classes" showBackButton ={true} showBackPath="/modules/users/teacher/ui/analytics"/>
                ...
                <tbody className="text-[11px]">
                  {currentData.map((schedule, index) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-2 text-left"> {schedule.classId || schedule.classLink}</td>
                        <td className="px-4 py-2 text-left font-medium text-[#3D8FDE] ">
                          {Array.isArray(schedule.student) && schedule.student.length > 0 ? (
    schedule.student.length === 1 ? (
      // ✅ SINGLE STUDENT → SAME UI
      (() => {
        const student = schedule.student[0]?.student;
      ...
      <>
        <button
          onClick={() =>
            setOpenEarningId(
              openEarningId === `${schedule._id}_${new Date(schedule.startDate).getTime()}_${schedule.startTime?.[0] || ''}` ? null : `${schedule._id}_${new Date(schedule.startDate).getTime()}_${schedule.startTime?.[0] || ''}`
            )
          }
          className="inline-flex items-center gap-1 px-2 py-1 rounded-md 
                     bg-[#42609a] text-white text-xs hover:bg-blue-700"
        >
          {schedule.student.length} Students
        </button>

        {openEarningId === `${schedule._id}_${new Date(schedule.startDate).getTime()}_${schedule.startTime?.[0] || ''}` && (
          <div
          ...
          >
            <ul className="space-y-1">
              {schedule.student.map((item: any, idx: number) => {
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="studentId"
                  className="w-full px-3 text-xs py-2 border dark:border-[#5c5c5c] rounded bg-transparent dark:bg-[#343434]"
              ...
              <div>
                ...
                <input
                  type="text"
                  name="studentName"
                  className="w-full px-3 text-xs dark:border-[#5c5c5c] py-2 border rounded bg-transparent dark:bg-[#343434]"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="date"
                    name="fromDate"
                    className="w-full dark:bg-[#343434] px-3 text-xs dark:border-[#5c5c5c] py-2 border rounded bg-transparent dark:[color-scheme:dark]"
                ...
                <div>
                  ...
                  <input
                    type="date"
                    name="toDate"
                    className="w-full dark:bg-[#343434] px-3 text-xs dark:border-[#5c5c5c] dark:[color-scheme:dark] py-2 border rounded bg-transparent"
              ...
              <div>
                ...
                <select
                  name="scheduleStatus"
                  className="w-full dark:bg-[#343434] px-3 text-xs dark:border-[#5c5c5c] py-2 border rounded bg-transparent"
