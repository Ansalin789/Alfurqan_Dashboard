...
const TrailManagement = () => {
  ...
  useEffect(() => {
    const root = document.getElementById("popup-root") as HTMLElement | null;
    setPopupRoot(root);
  }, []);
  ...
  return (
    <>
              ...
              <div className="flex flex-col gap-4 text-[13px]">
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      type="date"
                      className="w-full border border-gray-300 dark:border-[#5C5C5C] rounded-lg px-3 py-2 text-xs font-light text-gray-800 dark:text-white dark:bg-[#2B2B2B] focus:ring-2 focus:ring-[#576CBC] outline-none dark:[color-scheme:dark]"
                ...
                students
                  ...
                  .map((student, index) => (
                    <tr
                      ...
                      className={`dark:text-white ${index % 2 === 0
                        ? "bg-[#fff] dark:bg-[#2C2C2C]"
                        : "bg-[#F8F8F8] dark:bg-[#303030]"
                        } text-left text-[11px]`}
                    >
                      <td className="p-3 w-[9%] break-words">
                      ...
                      <td className="py-3 px-2 text-left">
                        {new Date(student.evaluation[0]?.joiningDate).toString() !==
                          "Invalid Date"
                          ? new Date(
      ...
      {/* PORTAL POPUP (renders outside table) */}
      {openPopup && popupRoot &&
        ReactDOM.createPortal(
          <div
            ref={popupRef}
            className="absolute w-32 mt-2 bg-white dark:bg-[#343434] shadow-md rounded-lg z-[9999] text-[11px]"
            style={{
          ...
          >
            <button
              className="w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-[#5c5c5c] rounded-t-lg"
              onClick={() => handleViewDetails(openPopup)}
            >
              View Details
            </button>

            <button className="w-full text-left px-4 py-2 dark:hover:bg-[#5c5c5c] rounded-b-lg hover:bg-gray-100" onClick={() => setOpenPopup(null)}>
              Cancel
            </button>
          </div>,
          popupRoot
