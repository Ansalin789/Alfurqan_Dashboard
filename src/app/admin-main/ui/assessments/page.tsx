...
export default function DashboardPage() {
    ...
    overallRating: 4,
    profileImage: "/assets/images/portrait-happy-smiling-young-businessman-blue-suit-isolated-white-wall.svg",
  };
...
<div className="w-full xl:w-2/3 mt-4 xl:mt-0">
          ...
          <ul className="text-sm space-y-2 text-white">
            <li className="flex justify-between">
              <span>Contact</span>
              <span className="text-[#DADADA]/80">{teacher.candidatePhoneNumber}</span>
            </li>
            <li className="flex justify-between">
              <span>Country</span>
              <span className="text-[#DADADA]/80">{teacher.candidateCountry}</span>
            </li>
            <li className="flex justify-between">
              <span>Role</span>
              <span className="text-[#DADADA]/80">{teacher.positionApplied}</span>
            </li>
            <li className="flex justify-between">
              <span>Level</span>
              <span className="text-[#DADADA]/80">{teacher.overallRating}</span>
            </li>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-3">
        {currentItems.map((user, i) => (
          ...
          >
            ...
            <div className="text-xs text-gray-500 dark:text-gray-300 mb-1">{user.level}</div>
            <div className="text-xs text-gray-500 dark:text-gray-300 mb-1">{user.language}</div>
    ...
    return (
      <div key={`pagination-${page}`} className="flex items-center">
        {Boolean (prevPage && page - prevPage > 1 && (
          <span key={`ellipsis-${page}`} className="px-2 text-sm text-gray-500 dark:text-gray-400">
            …
          </span>
        ))}
        <button
