  ...
  useEffect(() => {
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        // Map backend data to display names for UI
        if (json.data.roleAccess?.teachermodules) {
          const normalized = modules.reduce((acc, module) => {
            const key = getModuleKey(module);
            acc[module] = json.data.roleAccess.teachermodules[key] || false;
            return acc;
        ...
        if (json.data.roleAccess?.teachermodulesPermissions) {
          const normalizedPerms = modules.reduce((acc, module) => {
            const key = getModuleKey(module);
            acc[module] =
              json.data.roleAccess.teachermodulesPermissions[key] || {
                read: false,
                write: false,
                delete: false,
              };
            return acc;
          }, {} as { [key: string]: { read: boolean; write: boolean; delete: boolean } });
