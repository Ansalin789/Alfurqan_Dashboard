...
const SignIn: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const students = Object.values(data).filter(
        (item: any) => item && item._id // only valid student objects
      );
      console.log("[Student SignIn] Filtered Students:", students);
      const studentPortalData = students.map((s: any) => ({
        studentId: s._id,
      ...
      localStorage.setItem(
        "StudentPortalData",
        JSON.stringify(studentPortalData)
      );
      localStorage.setItem("StudentAuthToken", data.accessToken);
      const first = studentPortalData[0];
      console.log("StudentPortalId:", first.studentId);
      console.log("StudentcourseName:", first.course);
      console.log("StudentPortalName:", first.username);
      console.log("StudentPackage:", first.package);

      localStorage.setItem("StudentPortalId", first.studentId);
      localStorage.setItem("StudentcourseName", first.course);
      localStorage.setItem("StudentPortalName", first.username);
      localStorage.setItem("StudentPackage", first.package);

      if (data[0].role?.includes("Student")) {
  ...
  const handleGoogleSuccess = async (response: CredentialResponse) => {
    ...
    console.log("Google User Info:", emaildata);
      ...
      <div className="w-full lg:w-1/2 h-auto lg:h-screen bg-white flex flex-col overflow-hidden order-2 lg:order-1">
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 lg:px-8 overflow-auto scrollbar-none -mt-8 lg:-mt-16 py-8 lg:py-0">
          <div className="w-full max-w-md">
            <h2 className="text-2xl sm:text-[32px] font-bold text-black mb-2 text-center lg:text-left">
              Sign in
            </h2>
            <p className="text-[#718096] mb-6 sm:mb-8 text-sm sm:text-[14px] text-center lg:text-left">
              ...
              <button
                ...
                className="text-[#5A73B3] hover:text-[#4d6295] underline"
              >
                Create now
              </button>
            </p> 
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="mb-4">
                <label
                  htmlFor="username"
                  className="block text-sm font-medium text-[#718096] mb-1"
                >
                  Username
                </label>
                <input
              ...
              <div className="mb-6">
                <label
                  htmlFor="password"
                  className="block text-sm font-medium text-[#718096] mb-2"
                >
                  Password
                </label>
              ...
              <div className="flex items-center justify-between mb-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-xs sm:text-[12px] text-[#718096]">
                    Remember me
                  </span>
                </label>
                <a
                  href="#"
                  className="text-xs sm:text-[12px] text-[#5A73B3] hover:text-[#4d6295] underline"
                >
                  Forgot Password?
                </a>
              </div>
        ...
        <div className="relative z-10 flex flex-col justify-between w-full h-full px-4 sm:px-6 lg:px-12 py-6 sm:py-8">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 sm:gap-6">
              <div className="flex-1 text-left w-full">
                <h2 className="text-lg sm:text-xl lg:text-[22px] font-extrabold leading-[1.2] text-[#576CBC] mb-4 sm:mb-6">
                  Connecting You to Qur'an,
                  <br />
                  Arabic, and the Wisdom of
                  <br />
                  Islam
                </h2>
                <p className="text-[#808080] text-sm sm:text-[15px] lg:text-[17px] leading-relaxed mb-4 sm:mb-6">
                  ...
                  is there is any who will remember ?
                </p>
          ...
          <div className="text-center text-white mx-auto mb-8 sm:mb-12 mt-4 sm:mt-0">
            ...
            <p className="text-[#CFD9E0] text-[20px] sm:text-sm leading-relaxed max-w-xs sm:max-w-md mx-auto mb-2 sm:mb-4">
              Access comprehensive learning resources, track your academic
              progress, and engage with interactive educational content all in
              one platform.
            </p>
