...
useEffect(() => {
    const fetchData = async () => {
      ...
      // Group classes by month, counting total number of classes per month
      const monthlyClassCount: { [key: string]: number } = {};

      filteredData.forEach((item) => {
        const month = new Date(item.startDate).toLocaleString('default', { month: 'short' }); // Get month abbreviation (e.g., 'Jan', 'Feb')
        const studentId = item.student.studentId; // Unique student identifier
        ...
        monthlyClassCount[month] += 1; // Each entry is a class
      });

      // Convert the Set of unique students per month into an array for student data
      const formattedStudentData = Object.keys(monthlyStudentCount).map((month) => ({
        month,
        students: monthlyStudentCount[month].size, // Count of unique students
      }));

      // Convert the class count per month into an array for class data
      const formattedClassData = Object.keys(monthlyClassCount).map((month) => ({
        month,
        classes: monthlyClassCount[month], // Count of classes
      }));
...
const sortedClasses = [...filteredClasses].sort((a, b) => {
      ...
      <div className="p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6 w-[80%] ml-32">
            {/* Total Students Card */}
            <button
              className={`bg-white ${activeView === 'students' ? 'border-4' : 'border-2'} border-[#1C3557] rounded-xl p-4 shadow-lg cursor-pointer hover:shadow-md transition-shadow`}
            ...
            {/* Classes Card */}
            <button
              className={`bg-white ${activeView === 'classes' ? 'border-4' : 'border-2'} border-[#1C3557] rounded-xl p-4 shadow-lg cursor-pointer hover:shadow-md transition-shadow`}
            ...
            {/* Earnings Card */}
            <button
              className={`bg-white ${activeView === 'earnings' ? 'border-4' : 'border-2'} border-[#1C3557] rounded-xl p-4 shadow-lg cursor-pointer hover:shadow-md transition-shadow`}
          ...
          {/* List View */}
          <div className="bg-white rounded-xl shadow-sm mt-24">
            <div className="p-4 flex justify-between items-center">
              ...
              <div className="items-center gap-3 -mt-12">
                <button
                  className="flex items-center -mt-3 gap-2 ml-36 shadow-lg bg-[#FAFAFA] px-3 py-1.5 rounded-lg text-gray-600 hover:bg-gray-100 text-sm"
                  onClick={() => setIsFilterOpen(true)}
                >
                  <Filter className="w-4 h-4" />
                  <span>Filter</span>
                </button>
                <div className="relative shadow-ld mt-4 rounded-xl">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
                  <input
                    type="text"
                    placeholder="Search"
                    onChange={(e) => handleFilterChange('name', e.target.value)}
                    className="pl-9 pr-4 py-1.5 bg-[#FAFAFA] shadow-lg rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#4ade80] w-56"
                  />
                </div>
            ...
            {activeView === 'students' && (
              <table className="w-full">
                <thead>
                  <tr className="text-left text-[#374557]">
  ...
  <tr key={schedule.student.studentId} className="border-b border-gray-100 last:border-b-0">
    <td className="px-4 py-3">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 bg-[#DBDBDB] rounded-lg"></div>
        <span className="font-medium text-[#1e293b] text-sm">
          {schedule.student.studentFirstName} {schedule.student.studentLastName}
        </span>
      </div>
    </td>
    <td className="px-4 py-3 text-[#1e293b] text-sm">{schedule.student.studentId}</td>
    <td className="px-4 py-3 text-[#1e293b] text-sm">{schedule.package}</td>
    <td className="px-4 py-3 text-[#1e293b] text-sm">{schedule.scheduleStatus}</td>
    <td className="px-4 py-3 text-[#1e293b] text-sm">{new Date(schedule.startDate).toLocaleDateString()}</td>
    <td className="px-4 py-3">
    ...
    <td className="px-4 py-3">
      <span className="px-2.5 py-1 bg-[#4ade80]/10 text-[#299350] rounded-full text-xs">
        {schedule.status}
      </span>
    </td>
  </tr>
            ...
            {activeView === 'classes' && (
              <table className="w-full">
                <thead>
                  <tr className="text-left text-[#374557] border-y border-gray-100">
                    <th className="px-4 py-3 font-medium text-sm text-[#1e293b]">
                ...
                <tbody>
                  {sortedClasses.slice(0,3).map((cls) => (
                    <tr key={cls._id} className="border-b border-gray-100 last:border-b-0">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-gray-100 rounded-full"></div>
                          <span className="font-medium text-[#1e293b] text-sm">Trail Class</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{cls._id}</td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{cls.package}</td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{cls.scheduleStatus}</td>
                      <td className="px-4 py-3 text-[#1e293b] justify-center text-sm">{cls.totalHourse}</td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{new Date(cls.startDate).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
                ...
                <tbody>
                  {sortedEarnings.map((earning) => (
                    <tr key={earning.id} className="border-b border-gray-100 last:border-b-0">
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{earning.id}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-gray-100 rounded-full"></div>
                      ...
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{earning.courseType}</td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{earning.courseDuration}</td>
                      <td className="px-4 py-3 text-[#1e293b] text-sm">{earning.classDateTime}</td>
          ...
          <button
            className="text-green-500 mt-4 underline cursor-pointer bg-transparent border-none p-0"
            onClick={() => {
              if (activeView === 'students') {
