...
return (
  <BaseLayout>
      ...
      >
        ...
        <tbody>
          {currentItems.length === 0 ? (
            <tr>
              <td colSpan={7} className="text-center py-4">No meetings found.</td>
            </tr>
          ) : (
            currentItems.map((item: Meeting, index: number) => (
              <tr
                key={item._id}
                className={`text-[12px] ${
                  index % 2 === 0
                      ...
                      if (isStartMeetingNow(item.selectedDate, item.startTime, item.endTime)) {
                        return (
                          <button
                            ...
                            onClick={() =>
                              router.push(`/supervisor/ui/meetingvideocall?id=${item._id}`)
                            }
                          >
                            Start Meeting
                          </button>
    ...
    {showModal && (
      // ...rest of your modal JSX code here...
